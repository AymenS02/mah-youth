...
const BooksPage = () => {
  ...
  useEffect(() => {
    ...
    if (headerRef.current && booksGridRef.current) {
      const tl = gsap.timeline({ delay: 0.3 });
      gsap.set([headerRef.current, booksGridRef.current], { opacity: 0, y: 50 });
  ...
  const handleDownload = (book) => {
    if (book.downloadUrl) {
      ...
      alert("This book is not yet available for download.");
    }
  };
        ...
        <div className="container mx-auto px-6 max-w-7xl">
          ...
          <div ref={booksGridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredBooks.map((book) => (
              <div key={book._id} className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-2 overflow-hidden group">
