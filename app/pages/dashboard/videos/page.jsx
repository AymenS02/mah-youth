// app/pages/dashboard/videos/page.jsx
'use client';
import { useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
import Header from "/components/header/Header";
import { Video, Plus, Edit, Trash2, Search, Filter, ArrowLeft, Eye } from 'lucide-react';
...
export default function VideosManagement() {
  ...
  const filteredVideos = videos.filter(video => {
    ...
    const matchesFilter =
      filterStatus === 'all' ||
      (filterStatus === 'published' && video.videoUrl) ||
      (filterStatus === 'draft' && !video.videoUrl);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="pt-[200px] min-h-screen max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() => router.push('/pages/dashboard/videos/add')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVideos.map((video) => (
            <div key={video._id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition duration-200">
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <h3 className="text-lg font-bold text-gray-900 mb-2">{video.title}</h3>
                    <p className="text-sm text-gray-600 mb-1">by {video.author}</p>
                    <p className="text-sm text-gray-500 mb-1">{video.category}</p>
                  </div>
                  <span
                  ...
                  >
                    {video.videoUrl ? 'Published' : 'Draft'}
                  </span>
                </div>
                ...
                <div className="flex space-x-2">
                  <button
                    onClick={() => router.push(`/pages/dashboard/videos/${video._id}/edit`)}
